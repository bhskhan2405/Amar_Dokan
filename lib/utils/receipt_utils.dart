...
class ReceiptUtils {
  ...
  // --- 2. POS Bill Receipt (80mm - Centered Symmetric Design) ---
  ...
  static Future<void> generatePosReceipt({
    ...
    // Title Logic: If due > 0, it's a Credit Sale (বাকি বিক্রয়)
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              // Customer Details (Always Start Aligned)
              ...
              // Dynamic Receipt Title & Date (Perfect Centered)
              pw.Container(
                width: double.infinity,
                alignment: pw.Alignment.center,
                child: pw.Column(
  ...
  // --- 3. Other Reports (Statement, Accounts, Inventory) ---
        ...
        header: (context) => pw.Column(children: [
          pw.Row(mainAxisAlignment: pw.MainAxisAlignment.start, children: [
            ...
            pw.Expanded(child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
              _bt(shopInfo['name']!, font: shapedBold, fontSize: 32, color: PdfColors.blue900, align: ShapedTextAlign.center),
              if (shopInfo['address']!.isNotEmpty) _bt(shopInfo['address']!, font: shapedRegular, fontSize: 10, color: PdfColors.grey900, align: ShapedTextAlign.center),
              _bt('${_t('mobile')}: ${shopInfo['phone']}', font: shapedBold, fontSize: 11, align: ShapedTextAlign.center),
            ])),
        ...
        build: (context) => [
          ...
          pw.Table(
            ...
            children: [
              ...
              ...dataRows.map((row) => pw.TableRow(children: [
                _cell(row[0], flex: 1, font: shapedRegular), _cell(row[1], flex: 3, font: shapedRegular),
          ...
          pw.Row(mainAxisAlignment: pw.MainAxisAlignment.end, children: [
            pw.Container(padding: const pw.EdgeInsets.symmetric(horizontal: 15, vertical: 10), decoration: pw.BoxDecoration(color: PdfColors.blue50, border: pw.Border.all(color: PdfColors.blue900, width: 1), borderRadius: pw.BorderRadius.circular(5)), child: _bt('${_t('net_outstanding_due')} ৳${customerData['dueAmount']?.toStringAsFixed(2)}', font: shapedBold, fontSize: 13, color: PdfColors.red900)),
          ]),
        ],
      )
    );
  ...
  static Future<void> generateAccountsReport({required List<QueryDocumentSnapshot> sales, required List<QueryDocumentSnapshot> expenses, required List<QueryDocumentSnapshot> customerTransactions, required double totalSale, required double totalProfit, required double totalExpense, required double totalSalary, required double totalBonus, required DateTime start, required DateTime end}) async {
    final pdf = pw.Document();
    final shapedRegular = await _loadShapedFont("assets/fonts/SolaimanLipi-Normal.ttf", name: "SolReg");
    final shapedBold = await _loadShapedFont("assets/fonts/SolaimanLipi-Bold.ttf", name: "SolBold");
    final shopInfo = await getShopInfo();
    ...
    pdf.addPage(pw.MultiPage(pageFormat: PdfPageFormat.a4, header: (context) => pw.Column(children: [
      _bt(shopInfo['name']!, font: shapedBold, fontSize: 22, color: PdfColors.blue900, align: ShapedTextAlign.center),
      _bt(_t('accounts_report_title'), font: shapedBold, fontSize: 14, align: ShapedTextAlign.center),
      _bt('${_t('date_range')}: ${DateFormat('dd/MM/yyyy').format(start)} - ${DateFormat('dd/MM/yyyy').format(end)}', font: shapedRegular, fontSize: 10, align: ShapedTextAlign.center),
      pw.Divider(thickness: 1, color: PdfColors.blue900),
      pw.SizedBox(height: 10),
    ]), build: (context) => [
      _bt(_t('daily_trans_summary'), font: shapedBold, fontSize: 12),
      pw.SizedBox(height: 8),
      pw.Table(border: const pw.TableBorder(horizontalInside: pw.BorderSide(color: PdfColors.grey300, width: 0.5)), children: [
        pw.TableRow(decoration: const pw.BoxDecoration(color: PdfColors.blue800), children: [
          _cell(_t('date'), flex: 1, font: shapedBold, isHeader: true),
      ...
      pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceAround, children: [
        _summaryBoxSmallWithFont(_t('total_sales'), '৳${totalSale.toStringAsFixed(0)}', PdfColors.blue, shapedBold),
        _summaryBoxSmallWithFont(_t('total_profit'), '৳${totalProfit.toStringAsFixed(0)}', PdfColors.green, shapedBold),
        _summaryBoxSmallWithFont(_t('total_expense'), '৳${totalExpense.toStringAsFixed(0)}', PdfColors.red, shapedBold),
        _summaryBoxSmallWithFont(_t('salary'), '৳${combinedSalary.toStringAsFixed(0)}', PdfColors.orange, shapedBold),
      ]),
      pw.SizedBox(height: 20),
      pw.Center(child: pw.Container(padding: const pw.EdgeInsets.all(15), decoration: pw.BoxDecoration(color: netProfit >= 0 ? PdfColors.green50 : PdfColors.red50, border: pw.Border.all(color: netProfit >= 0 ? PdfColors.green : PdfColors.red, width: 2), borderRadius: pw.BorderRadius.circular(10)), child: pw.Column(children: [
        _bt(_t('profit'), font: shapedBold, fontSize: 14, color: PdfColors.green900, align: ShapedTextAlign.center),
        _bt('৳${netProfit.toStringAsFixed(2)}', font: shapedBold, fontSize: 20, color: netProfit >= 0 ? PdfColors.green900 : PdfColors.red900, align: ShapedTextAlign.center),
      ]))),
    ]));
    await Printing.layoutPdf(onLayout: (PdfPageFormat format) async => pdf.save());
  }

  static Future<void> generateInventoryReport({required List<Map<String, dynamic>> logs, required DateTime? start, required DateTime? end}) async {
    final pdf = pw.Document();
    final fontRegular = await _loadFont("assets/fonts/SolaimanLipi-Normal.ttf");
    final shapedRegular = await _loadShapedFont("assets/fonts/SolaimanLipi-Normal.ttf", name: "SolReg");
    final shapedBold = await _loadShapedFont("assets/fonts/SolaimanLipi-Bold.ttf", name: "SolBold");
    ...
    String dateRange = (start != null && end != null) ? (start == end ? DateFormat('dd/MM/yyyy').format(start) : "${DateFormat('dd/MM/yyyy').format(start)} - ${DateFormat('dd/MM/yyyy').format(end)}") : _t("Daily Transaction Summary");
    ...
    pdf.addPage(pw.MultiPage(pageFormat: PdfPageFormat.a4, header: (context) => pw.Column(children: [
      pw.Row(mainAxisAlignment: pw.MainAxisAlignment.start, children: [
        ...
        pw.Expanded(child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
          _bt(shopInfo['name']!, font: shapedBold, fontSize: 28, color: PdfColors.blue900, align: ShapedTextAlign.center),
          if (shopInfo['address']!.isNotEmpty) _bt(shopInfo['address']!, font: shapedRegular, fontSize: 10, align: ShapedTextAlign.center),
          _bt('${_t('mobile')}: ${shopInfo['phone']}', font: shapedBold, fontSize: 10, align: ShapedTextAlign.center),
        ])),
      ...
      pw.SizedBox(height: 15),
      pw.Table(border: const pw.TableBorder(horizontalInside: pw.BorderSide(color: PdfColors.grey300, width: 0.5), bottom: pw.BorderSide(color: PdfColors.grey300, width: 0.5)), children: [
        pw.TableRow(decoration: const pw.BoxDecoration(color: PdfColors.blue800), children: [
          _cell(_t('date'), flex: 1, font: shapedBold, isHeader: true),
  ...
  static Future<void> generateSingleAccountPdf({required Map<String, dynamic> data, required String timeString, bool isExpense = false, bool isShare = false}) async {
    final pdf = pw.Document();
    final fontRegular = await _loadFont("assets/fonts/SolaimanLipi-Normal.ttf");
    final shapedBold = await _loadShapedFont("assets/fonts/SolaimanLipi-Bold.ttf", name: "SolBold");
    final shopInfo = await getShopInfo();
    ...
    pdf.addPage(pw.Page(pageFormat: PdfPageFormat.a4, build: (context) => pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
      ...
      pw.Spacer(),
      pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
        pw.Column(children: [pw.SizedBox(width: 100, child: pw.Divider()), _bt(_t('authorized_sign'), font: shapedBold, fontSize: 10, align: ShapedTextAlign.center)]),
        pw.Column(children: [pw.SizedBox(width: 100, child: pw.Divider()), _bt(_t('receiver_sign'), font: shapedBold, fontSize: 10, align: ShapedTextAlign.center)]),
      ]),
    ])));
